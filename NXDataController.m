...
NSString * const NXDataControllerErrorDomain = @"NXDataControllerErrorDomain";

const NSInteger NXDataControllerErrorInvalidObject = 1;

NSString * const NXResponseObjectKey = @"NXResponseObjectKey";

@interface NXDataController ()
...
@implementation NXDataController

- (id)copyWithZone:(NSZone *)zone
{
    NXDataController *copy = [[[self class] allocWithZone:zone] initWithDelegate:self.delegate];
...
+ (instancetype)sharedDataController
{
    ...
    NXDataController *aController = nil;
...
- (void)requestWithParams:(NSDictionary *)params
                  success:(NXRequestSuccessBlock)success
                  failure:(NXRequestFailureBlock)failure
{
    if (success) {
...
- (void)requestMore
{
    ...
    // 默认插入模式
    self.mantleObjectListUpdateStyle = NXMantleObjectListUpdateInsert;
...
- (void)requestOpertaionSuccess:(NSURLSessionDataTask *)task responseObject:(id)responseObject
{
    ...
    if ([self isValidWithObject:responseObject]) {
        ...
        if (responseObject) {
            error = [NSError errorWithDomain:NXDataControllerErrorDomain
                                        code:NXDataControllerErrorInvalidObject
                                    userInfo:@{ NXResponseObjectKey : responseObject }];
        }
...
- (void)logWithObject:(id)object
{
    if (self.logBlock) {
        ...
        NSString *fullLog = [NSString stringWithFormat:@"NXDataController: %@", object];
...
- (void)handleMantleObjectListWithArray:(NSArray *)array mantleClass:(Class)mantleClass
{
    ...
    if (models) {
        if (self.mantleObjectListUpdateStyle == NXMantleObjectListUpdateInsert) {
            if (self.mantleObjectList == nil) {
                self.mantleObjectList = [models mutableCopy];
            } else {
                [self.mantleObjectList addObjectsFromArray:models];
            }
        } else if (self.mantleObjectListUpdateStyle == NXMantleObjectListUpdateReplace) {
            self.mantleObjectList = [models mutableCopy];
        }
    }
}
